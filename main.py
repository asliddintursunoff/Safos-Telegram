...
from handlers.order import buyurtma_handler
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE):
    """Log every unexpected error and tell the user, instead of silently doing nothing."""
    logger.error("Unhandled error while processing update", exc_info=context.error)
    if isinstance(update, Update) and update.effective_chat:
        try:
            await context.bot.send_message(update.effective_chat.id, "❌ Kutilmagan xatolik yuz berdi. Iltimos qaytadan urinib ko'ring.")
        except Exception:
            pass
...
# order conversation (new + edit) first, so /start typed in the middle of an order ends it cleanly
